...
%				\fonte{O autor (\the\year)}
		...
		\begin{itemize}
			...
			\note{a: escala de tempo da variável de recuperação. Valores pequenos = recuperação mais lenta}
			\note{b: sensibilidade da var. de recuperação. Valores maiores agrupam mais $v$ e $u$}
			\note{c: valor de reset do potencial de membrana}
			\note{d: valor de reset da variável de recuperação}
		\end{itemize}
	\end{columns}
	\[
		\begin{aligned}
